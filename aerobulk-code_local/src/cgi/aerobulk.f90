...
CONTAINS
  ...
  SUBROUTINE aerobulk_init(psst, pt, pq, pU, pV, pslp)
    ...
    !PRINT *, '    *** seting jpi and jpj to ', jpi, jpj
    
    ! 2.
    ni = size(pt,1)  ; nj = size(pt,2)
    IF ( (ni /= jpi).OR.(ni /= jpi) ) THEN
       !PRINT *, 'ERROR: aerobulk_init => SST and t_air arrays do not agree in shape!' ;
       STOP
    END IF
  ...
  SUBROUTINE aerobulk_model(calgo, zt, zu, sst, t_zt, &
    ...
    !! OPTIONAL :
    !! ----------
    !!    * Nitter  : number of itterattions in the bulk algorithm (default is 4)
    !!
    !!============================================================================
    ...
    INTEGER, INTENT(in), OPTIONAL :: Nitter

    IF ( present(Nitter) ) nb_itt = Nitter  ! Updating number of itterations (define in mod_const)
