PROGRAM cx_vs_wind_test
  ...
  REAL, DIMENSION(1,1) :: sst, sstk, qsat_sst, sst_v, slp, t10, w10, q10, Ublk, &
       &  z0, z0t, z0q, us, ts, qs, &
  ...
  REAL, DIMENSION(n_w) :: &
       ...
       &   t_us, t_ts, t_qs, &
       &   t_ra
  !       &   t_tau, t_evap, t_qsens, t_zeta

  IF (iargc() < 1 ) THEN
     PRINT *, 'USAGE: cx_vs_wind_test.x <algo (coare/ncar)> <SST (deg.C)> (charn constant? => <T>)'
     STOP
  END IF
  ...
  IF (iargc() == 3 ) THEN
     CALL getarg(3,cc)
     IF ( (cc == 'T').OR.(cc == 't') ) THEN
        l_use_cst_charnock = .TRUE.
        PRINT *, ''
        PRINT *, '   *** Will use a constant Charnock parameter!'
        PRINT *, ''
        cextra = '_cst_charn'
     ELSE
        PRINT *, 'USAGE: cx_vs_wind_test.x <algo (coare/ncar)> <SST (deg.C)> (charn constant? => <T>)'
  ...
  DO jdt = 1, ndt
     ...
     IF (Dtv >= 0.) THEN
        ...
        !WRITE(cf_ra,'("dat/ra_dtv_+",i4.4,"_sst_",i2.2,"_",a,a,".dat")') int(100*Dtv), int(sst), trim(calgo), trim(cextra)
     ...
     ELSE
        ...
        !WRITE(cf_ra,'("dat/ra_dtv_",i5.4,"_sst_",i2.2,"_",a,a,".dat")') int(100*Dtv), int(sst), trim(calgo), trim(cextra)
     ...
     DO jw = 1, n_w
        ...
        END IF
        !!
        mCd = 0. ;  mCe = 0. ;  mCh = 0. ; mzeta = 0. ; mz0 = 0. ; mz0t = 0. ; mz0q = 0. ; mra = 0.
        mus = 0. ;  mts = 0. ;  mqs = 0.
        !!
        DO jh = 1, nrh
           !!
           ...
           IF ( trim(calgo) == 'coare' ) &
                CALL TURB_COARE_2Z(zt, zu, sstk, XT_a(jdt,jh), qsat_sst, XQ_a(jdt,jh), w10, &
                &          Cd, Ch, Ce, t10, q10, Ublk, l_charn_cst=l_use_cst_charnock)

           IF ( trim(calgo) == 'ncar' ) &
                CALL TURB_NCAR_2Z( zt, zu, sstk, XT_a(jdt,jh), qsat_sst, XQ_a(jdt,jh), w10, &
                &          Cd, Ch, Ce, t10, q10, Ublk)
           !!
           !!
           mCd = mCd + Cd/nrh
           mCh = mCh + Ch/nrh
...
201 CONTINUE
  ...
  DO WHILE ( ABS(dt) > 1.E-9 )
     ...
     IF ( trim(calgo) == 'coare' ) &
          CALL TURB_COARE_2Z(zt, zu, sstk, v_tq(1,1), qsat_sst, v_tq(2,1), w10, &
          &             Cd, Ch, Ce, t10, q10, Ublk)

     IF ( trim(calgo) == 'ncar' ) &
          CALL TURB_NCAR_2Z(zt, zu, sstk, v_tq(1,1), qsat_sst, v_tq(2,1), w10, &
          &             Cd, Ch, Ce, t10, q10, Ublk)
