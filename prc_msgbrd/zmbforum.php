...
<script type="text/javascript">
 ...
 $("document").ready(function() {
 	...
 	function sendInfoToServer() {
 		$('span.commentSent').load('sendInfoToServer.php', $('#theForm').serializeArray());
 	}
      ...
      if (isset($_POST['submitted'])) { // Check if the form has been submitted.
		...
		if ($u && $p && $captchchk) {
			...
			if (mysql_affected_rows() == 1) { // A match was made
				...
				if ($row[6] != NULL) 
				{
					header("Location: http://localhost/prc_msgbrd/mbforgotpass.php");
				...
				// Redirect the user
				header("Location: http://localhost/prc_msgbrd/mblogin.php");
				mysql_close(); // Close the database connection.
				exit();
			}
